...
//! chat-authoring tools (`create_automation` / `create_work_task`) and the
//! work-task orchestration tools (`split_work_task` / `list_work_tasks` /
//! `start_work_task` / `cancel_work_task`), gated by the
